...
def get_trace_data(log_path, trace):
    ...
    # for test
    # log_trace_path = log_path
    if trace == "128-128":
        # qps_list = [32.0, 33.0, 34.0, 35.0, 36.0]
        qps_list = [32.0, 33.0, 34.0, 35.0, 36.0, 37.0, 38.0]
    elif trace == "256-256":
        # qps_list = [7.4, 7.5, 7.6, 7.7, 7.8]
        qps_list = [7.4, 7.5, 7.6, 7.7, 7.8, 7.9, 8.0]
    elif trace == "512-512":
        # qps_list = [2.6, 2.7, 2.8, 2.9, 3.0]
        qps_list = [2.6, 2.7, 2.8, 2.9, 3.0, 3.1, 3.2]
    elif trace == "128-512":
        # qps_list = [4.10, 4.15, 4.20, 4.25, 4.30]
        qps_list = [4.10, 4.15, 4.20, 4.25, 4.30, 4.35, 4.40]
    elif trace == "512-128":
        # qps_list = [10.0, 12.0, 14.0, 16.0, 18.0]
        qps_list = [10.0, 12.0, 14.0, 16.0, 18.0, 20.0, 22.0]
    else:
        assert("figure11 onlys contains 5 traces: 128-128, 256-256, 512-512, 128-512, 512-128")
    trace_data = {}
    for qps in qps_list:
        trace_data[qps] = {
            'INFaaS++' : {
...
def plot_figure11_paper(log_path):
    wspace = 0.35
    hspace = 0.3
    fig, axs = plt.subplots(nrows=5, ncols=3 * 2 + 1, figsize=(3.0 * 7 + wspace * 6, 2.2 * 5 + hspace * 4))
    plt.subplots_adjust(wspace=wspace, hspace=hspace)

    if not isinstance(axs, np.ndarray):
        axs = np.array([axs])
    axs_dict = {}
    traces = ["128-128", "256-256", "512-512", "128-512", "512-128"]
    ...
    for trace in traces:
        ...
        # for test
        # from raw_data_paper import get_evaluation1_data
        # qps_list, infaaspp_latency_dict, llumnix_latency_dict, round_robin_latency_dict = get_evaluation1_data(trace)
        # import pdb; pdb.set_trace()
